...
abstract public class ModbusMaster extends Modbus {
	...
	/**
	 * If the slave equipment only supports multiple write commands, set this to
	 * true. Otherwise, and combination of single or multiple write commands
	...
	/**
	 * An input stream ePoll will use a single thread to read all input streams.
	 * If multiple serial or TCP modbus connections are to be made, an ePoll can
	...
	/**
	 * Returns a value from the modbus network according to the given locator
	 * information. Various data types are allowed to be requested including
	 * multi-word types. The determination of the correct request message to
	 * send is handled automatically.
	 * 
	 * @param locator
	 *            the information required to locate the value in the modbus
	 *            network.
	 * @return an object representing the value found. This will be one of
	 *         Boolean, Short, Integer, Long, BigInteger, Float, or Double. See
	 *         the DataType enumeration for details on which type to expect.
	 * @throws ModbusTransportException
	 *             if there was an IO error or other technical failure while
	 *             sending the message
	...
	public <T> T getValue(BaseLocator<T> locator) throws ModbusTransportException, ErrorResponseException {
		...
		MultiSlaveBatchResults result = send(batch);
		if (result.getBatchResults(locator.getSlaveId()+".0") != null)
			return (T) result.getBatchResults(locator.getSlaveId()+".0").getValue("");
		return null;
	}
	 ...
	 * multi-word types. The determination of the correct write message to send
	 * is handled automatically.
	 * 
	 * @param locator
	 *            the information required to locate the value in the modbus
	 *            network.
	 * @value an object representing the value to be set. This will be one of
	 *        Boolean, Short, Integer, Long, BigInteger, Float, or Double. See
	 *        the DataType enumeration for details on which type to expect.
	 * @throws ModbusTransportException
	 *             if there was an IO error or other technical failure while
	...
	/**
	 * Node scanning. Returns a list of slave nodes that respond to a read
	 * exception status request (perhaps with an error, but respond
	 * nonetheless).
	 * 
	 * Note: a similar scan could be done for registers in nodes, but, for one
	 * thing, it would take some time to run, and in any case the results would
	 * not be meaningful since there would be no semantic information
	...
	/**
	 * Useful for sending a number of polling commands at once, or at least in
	 * as optimal a batch as possible.
	 * @throws ModbusTransportException 
	 * @throws ErrorResponseException 
	 */
	public MultiSlaveBatchResults send(BatchRead batch) throws ModbusTransportException, ErrorResponseException {
		if (!initialized)
			throw new ModbusTransportException("not initialized");

		MultiSlaveBatchResults multiSlaveBatchResults = new MultiSlaveBatchResults();
		List<ReadFunctionGroup> functionGroups = batch.getReadFunctionGroups(this);
		...
		for (ReadFunctionGroup functionGroup : functionGroups) {
			...
			try {
				...
				multiSlaveBatchResults.addBatchResults(functionGroup.getSlaveAndRange().getSlaveId()+"."+(i++), results);
			}
			if (batch.isCancel())
				break;
		}

		return multiSlaveBatchResults;
	}
	...
	//
	/**
	 * This method assumes that all locators have already been pre-sorted and
	 * grouped into valid requests, say, by the createRequestGroups method.
	 */
	private void sendFunctionGroup(ReadFunctionGroup functionGroup, BatchResults results, boolean errorsInResults,
		...
		try {
			...
			for (KeyedModbusLocator locator : functionGroup.getLocators())
				results.addResult(locator.getKey(), "error");
	...
	private void setHoldingRegisterBit(int slaveId, int writeOffset, int bit, boolean value)
			throws ModbusTransportException, ErrorResponseException {
		// Writing a bit in a holding register field. There are two ways to do
		// this. The easy way is to
		// use a write mask request, but it is not always supported. The hard
		// way is to read the value, change
		// the appropriate bit, and then write it back again (so as not to
		// overwrite the other bits in the
		// value). However, since the hard way is not atomic, it is not
		// fail-safe either, but it should be
		// at least possible.
		...
		if (sp.getWriteMaskRegister()) {
			...
			if (response.getExceptionCode() == ExceptionCode.ILLEGAL_FUNCTION)
				// The function is probably not supported. Fail-over to the two
				// step.
